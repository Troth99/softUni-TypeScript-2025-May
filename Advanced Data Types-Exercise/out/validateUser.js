...
function validateUser(user) {
    ...
    const validId = (typeof userValidation.id === 'number' && userValidation.id > 100) ||
        (typeof userValidation.id === 'string' && userValidation.id.length === 14);
    const validUserName = typeof userValidation.username === 'string' && userValidation.username.length >= 5 && userValidation.username.length <= 10;
    const userPassword = (typeof userValidation.passwordHash === 'string' && userValidation.passwordHash.length === 20) ||
    ...
    const statusValidation = userValidation.status.includes('Locked') ||
