...
//decorator for all users 
function getUsersDecorator(seconds: number){
    return function (target:any, methodName: string, descriptor: PropertyDescriptor){
        const originalValue = descriptor.value 

        descriptor.value = function() {

            const allItems = originalValue.call(this)
...
function censoredProperties<T extends { [key: string]: any }>(censorService: MockCensorService <T>){
    return function(target:any, methodName: string, descriptor: PropertyDescriptor){
        const originalMethod = descriptor.value 

        descriptor.value = function(){
            const allItems = originalMethod.call(this)
            const censor = censorService.censorProperties(allItems);
            return censor
        }
        return descriptor
    }
...
class UsersService {
    ...
    @getUsersDecorator(5)
    @censoredProperties(userCensorService)
    getUsers() { return this._users; }

    @log
    @getUsersDecorator(10)
    @censoredProperties(employeeCensorService)
